...
ASSASSIN_DESC = "Does nothing when played, but if in player's hand when a Guard is used on them, the player who used the Guard is eliminated, even if they correctly guessed. Then the player with the Assassin must draw a replacement card."
JESTER_DESC = "Player chooses another player and gives them a 'jester token'. If a player with a 'jester token' wins the round, then the player who played the Jester gets an additional token of affection."
CARDINAL_DESC = "Player chooses two players and they swap hands, then looks at the hand of one of those players."
BARONESS_DESC = "Player looks at one or two other players' hands."
SYCOPHANT_DESC = "Player chooses a player. During the next turn, players must target that player with card effects if able."
COUNT_DESC = "Does nothing when played. If in a player's discard pile at the end of a round, that player receives a +1 bonus to their card's value at the end of the round."
CONSTABLE_DESC = "Does nothing when played. If in a player's discard pile and that player is eliminated from the round via a card effect, then that player gets a token of affection."
DOWAGER_QUEEN_DESC = "The player will privately compare hands with another player, and the player with the higher-valued card is eliminated from the round."
BISHOP_DESC = "The player must guess the number of a card in another player's hand. If guessed correctly, then the player who played the Bishop gets a token of affection and the player that was designated may optionally choose to replace their card with a new one. Despite this card's value of '9' and the Princess's value of '8', the Bishop loses to the Princess at the end of the round."
...
class LLDeck:
	def __init__(self, premium=False):
		...
		if premium:
			self.cards.extend([
				LLCard("Assassin",ASSASSIN_DESC,0),
				LLCard("Jester",JESTER_DESC,0),
				LLCard("Guard",GUARD_DESC,1),
				LLCard("Guard",GUARD_DESC,1),
				LLCard("Guard",GUARD_DESC,1),
				LLCard("Cardinal",CARDINAL_DESC,2),
				LLCard("Cardinal",CARDINAL_DESC,2),
				LLCard("Baroness",BARONESS_DESC,3),
				LLCard("Baroness",BARONESS_DESC,3),
				LLCard("Sycophant",SYCOPHANT_DESC,4),
				LLCard("Sycophant",SYCOPHANT_DESC,4),
				LLCard("Count",COUNT_DESC,5),
				LLCard("Count",COUNT_DESC,5),
				LLCard("Constable",CONSTABLE_DESC,6),
				LLCard("Dowager Queen",DOWAGER_QUEEN_DESC,7),
				LLCard("Bishop",BISHOP_DESC,9),
			])
...
class LLPlayer:
	def __init__(self, name=None, discordUser=None):
		...
		self.targeted = None
		self.jesterTokenByUser = None
		self.firstTarget = None
		self.sycophant = False
...
class LLGame:
	...
	def __init__(self, users, premium=False):
		self.messages = []
		self.players = []
		# Force the game to be premium if there are too many players
		if len(users) > 4:
			premium = True
	...
	def ActionTargetting(self, player, text):
		...
		# ###################### #
		# Jester was played last #
		# ###################### #
		if player.played[-1].name == "Jester":
			self.messages.append({'who':'all','message':"A Jester token has been put on " + player.targeted.name})
			player.targeted.jesterTokenByUser = player
			
		# ##################### #
		# Guard was played last #
		# ##################### #
		elif player.played[-1].name == "Guard":
		...
		# ######################## #
		# Cardinal was played last #
		# ######################## #	
		elif player.played[-1].name == 'Cardinal':
			if player.firstTarget is None:
				player.firstTarget = player.targeted 
				player.targeted = None
				self.messages.append({'who':player,'message':"Please target a second player by using one of the following numbers"})
				targetplayers = self.GetTargetablePlayers(player)
				for i in range(len(targetplayers)):
					self.messages.append({'who':'all','message':str(i+1) + ": " + targetplayers[i].name})
				return
			else:
				self.messages.append({'who':'all','message':player.targeted.name + " and " + player.firstTarget + "have switched hands"})
				temp = player.firstTarget.hand[0]
				player.firstTarget.hand[0] = player.targeted.hand[0]
				player.targeted.hand[0] = temp
				self.messages.append({'who':player,'message':player.targeted.name + " has a " + player.targeted.hand[0].name})	
				self.messages.append({'who':player.targeted,'message':"You have a new hand of " + player.targeted.hand[0].name})
				self.messages.append({'who':player.firstTarget,'message':"You have a new hand of " + player.firstTarget.hand[0].name})
		...
		# ######################## #
		# Baroness was played last #
		# ######################## #
		elif player.played[-1].name == 'Baroness':
			if player.firstTarget is None:
				player.firstTarget = player.targeted 
				player.targeted = None
				self.messages.append({'who':player,'message':"Please target a second player by using one of the following numbers"})
				...
				self.messages.append({'who':player,'message':player.targeted.name + " has a hand of " + player.targeted.hand[0].name})
				self.messages.append({'who':player,'message':player.firstTarget.name + " has a hand of " + player.firstTarget.hand[0].name})
				
		# ######################### #
		# Sycophant was played last #
		# ######################### #
		elif player.played[-1].name == 'Sycophant':
			player.targeted.sycophant = True
		...
		# ############################# #
		# Dowager Queen was played last #
		# ############################# 
		elif player.played[-1].name == "Dowager Queen":
			if player.targeted.hand[0].value < player.hand[0].value:
				self.messages.append({'who':'all','message':player.name + " is now out"})
				player.out = True
			elif player.hand[0].value < player.targeted.hand[0].value:
